                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" class="form-control sort-order w-50" placeholder="0"
                                    ...
                                    value="{{ $business->sort_order }}">
                            </td>
                            <td>
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input status" id="status{{ base64_encode($business->id) }}" name="status"
                                        data-model="Business" value="{{ $business->status }}"
                                        data-id="{{ base64_encode($business->id) }}" @checked($business->status == 1)>
                                    <label class="custom-control-label"
                                        for="status{{ base64_encode($business->id) }}">{{ $business->status == 1 ? 'Enabled' : 'Disabled' }}</label>
                                </div>
                            </td>
                            <td>
                                <a href="{{ route('business.edit', base64_encode($business->id)) }}"
